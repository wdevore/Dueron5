...
func NewSurgeGraph(renderer *sdl.Renderer, texture *sdl.Texture, width, height int) IGraph {
	g := new(SurgeGraph)
...
func (g *SurgeGraph) Listen(msg *comm.MessageEvent) {
	if !g.selected {
		return
	}
...
func (g *SurgeGraph) Handle(vx, vy int32, eventType events.MouseEventType) (handled bool, id int) {
	...
	case events.MouseButton:
		if inside {
			...
			if g.selected {
				...
				comm.MsgBus.Send2("SurgeGraph", "Graph", "Selected", "Surge", fmt.Sprintf("%d", g.ID()), "")
				// Update gui Range fields the sample's range.
				samples := samples.Sim.SurgeSamples
				start, end := samples.GetRange()

				// Send message to panel including the panel's id.
				comm.MsgBus.Send3("SurgeGraph", "Model", "Set", "", "", "Lane_Start", fmt.Sprintf("%d", start))
				comm.MsgBus.Send3("SurgeGraph", "Model", "Set", "", "", "Lane_End", fmt.Sprintf("%d", end))
			} else {
				comm.MsgBus.Send2("SurgeGraph", "Graph", "UnSelected", "Surge", fmt.Sprintf("%d", g.ID()), "")
			}
			return true, g.ID()
		}
		break
...
func (g *SurgeGraph) SetSeries(accessor SeriesAccessor) {
}

// Destroy release resources
func (g *SurgeGraph) Destroy() {
}

func (g *SurgeGraph) Prep() {
}

// DrawAt renders graph to texture
func (g *SurgeGraph) Draw() {
...
func (g *SurgeGraph) Check() bool {
	if samples.Sim.SurgeSamples == nil {
		return false
	}

	lanes := samples.Sim.SurgeSamples.GetLanes()
	...
	if g.graphIt.First() {
		...
		g.setScanWindow(samples.Sim.SurgeSamples)
		return true
	}
...
func (g *SurgeGraph) setScanWindow(lanes *samples.Samples) {
...
func (g *SurgeGraph) dataAccessor() (x, y float64, c color.Color, more int) {
