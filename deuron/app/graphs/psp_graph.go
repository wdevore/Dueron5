...
func NewPspGraph(renderer *sdl.Renderer, texture *sdl.Texture, width, height int) IGraph {
	g := new(PspGraph)
...
func (g *PspGraph) Listen(msg *comm.MessageEvent) {
	if !g.selected {
		return
	}

	samples := samples.Sim.PspSamples
...
func (g *PspGraph) Handle(vx, vy int32, eventType events.MouseEventType) (handled bool, id int) {
	...
	case events.MouseButton:
		if inside {
			...
			if g.selected {
				...
				comm.MsgBus.Send2("PspGraph", "Graph", "Selected", "Surge", fmt.Sprintf("%d", g.ID()), "")
				// Update gui Range fields the sample's range.
				samples := samples.Sim.PspSamples
				start, end := samples.GetRange()

				// Send message to panel including the panel's id.
				comm.MsgBus.Send3("PspGraph", "Model", "Set", "", "", "Lane_Start", fmt.Sprintf("%d", start))
				comm.MsgBus.Send3("PspGraph", "Model", "Set", "", "", "Lane_End", fmt.Sprintf("%d", end))
			} else {
				comm.MsgBus.Send2("PspGraph", "Graph", "UnSelected", "Surge", fmt.Sprintf("%d", g.ID()), "")
			}
			return true, g.ID()
		}
		break
...
func (g *PspGraph) SetSeries(accessor SeriesAccessor) {
}

// Destroy release resources
func (g *PspGraph) Destroy() {
}

func (g *PspGraph) Prep() {
}

// DrawAt renders graph to texture
func (g *PspGraph) Draw() {
...
func (g *PspGraph) Check() bool {
	if samples.Sim.PspSamples == nil {
		return false
	}

	lanes := samples.Sim.PspSamples.GetLanes()
	...
	if g.graphIt.First() {
		...
		g.setScanWindow(samples.Sim.PspSamples)
		return true
	}
...
func (g *PspGraph) setScanWindow(lanes *samples.Samples) {
...
func (g *PspGraph) dataAccessor() (x, y float64, c color.Color, more int) {
