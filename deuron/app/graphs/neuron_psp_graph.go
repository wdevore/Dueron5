...
type NeuronPspGraph struct {
	BaseGraph
	gui.BaseWidget

	lineColor      color.RGBA
	altLineColor   color.RGBA
	thresholdColor color.RGBA
	even           bool
...
func NewNeuronPspGraph(renderer *sdl.Renderer, texture *sdl.Texture, width, height int) IGraph {
	g := new(NeuronPspGraph)
	...
	g.thresholdColor = color.RGBA{64, 127, 255, 255}

	return g
}

func (g *NeuronPspGraph) Listen(msg *comm.MessageEvent) {
	if !g.selected {
		return
	}

	samples := samples.Sim.NeuronPspSamples
...
func (g *NeuronPspGraph) Handle(vx, vy int32, eventType events.MouseEventType) (handled bool, id int) {
	...
	case events.MouseButton:
		if inside {
			...
			if g.selected {
				...
				comm.MsgBus.Send2("NeuronPspGraph", "Graph", "Selected", "Surge", fmt.Sprintf("%d", g.ID()), "")
				// Update gui Range fields the sample's range.
				samples := samples.Sim.NeuronPspSamples
				start, end := samples.GetRange()

				// Send message to panel including the panel's id.
				comm.MsgBus.Send3("NeuronPspGraph", "Model", "Set", "", "", "Lane_Start", fmt.Sprintf("%d", start))
				comm.MsgBus.Send3("NeuronPspGraph", "Model", "Set", "", "", "Lane_End", fmt.Sprintf("%d", end))
			} else {
				comm.MsgBus.Send2("NeuronPspGraph", "Graph", "UnSelected", "Surge", fmt.Sprintf("%d", g.ID()), "")
			}
			return true, g.ID()
		}
		break
...
func (g *NeuronPspGraph) SetSeries(accessor SeriesAccessor) {
}

// Destroy release resources
func (g *NeuronPspGraph) Destroy() {
}

func (g *NeuronPspGraph) Prep() {
}

// DrawAt renders graph to texture
func (g *NeuronPspGraph) Draw() {
	...
	uspY := g.Linear(g.activeLane.Min, g.activeLane.Max, threshold)
	winY := g.Lerp(0, g.upperY, uspY)
	g.DC.SetColor(g.thresholdColor)
	g.DC.MoveTo(0, winY)
	g.DC.LineTo(g.upperX, winY)
	g.DC.Stroke()
...
func (g *NeuronPspGraph) Check() bool {
	if samples.Sim.NeuronPspSamples == nil {
		return false
	}

	lanes := samples.Sim.NeuronPspSamples.GetLanes()
	...
	if g.graphIt.First() {
		...
		g.setScanWindow(samples.Sim.NeuronPspSamples)
		return true
	}
...
func (g *NeuronPspGraph) setScanWindow(lanes *samples.Samples) {
...
func (g *NeuronPspGraph) dataAccessor() (x, y float64, c color.Color, more int) {
