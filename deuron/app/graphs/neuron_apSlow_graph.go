...
func NewNeuronAPSlowGraph(renderer *sdl.Renderer, texture *sdl.Texture, width, height int) IGraph {
	g := new(NeuronAPSlowGraph)
...
func (g *NeuronAPSlowGraph) Listen(msg *comm.MessageEvent) {
	...
	samples := samples.Sim.NeuronAPSlowSamples
...
func (g *NeuronAPSlowGraph) Handle(vx, vy int32, eventType events.MouseEventType) (handled bool, id int) {
	...
	case events.MouseButton:
		if inside {
			...
			if g.selected {
				...
				comm.MsgBus.Send2("NeuronAPSlowGraph", "Graph", "Selected", "Surge", fmt.Sprintf("%d", g.ID()), "")
				// Update gui Range fields the sample's range.
				samples := samples.Sim.NeuronAPSlowSamples
				start, end := samples.GetRange()

				// Send message to panel including the panel's id.
				comm.MsgBus.Send3("NeuronAPSlowGraph", "Model", "Set", "", "", "Lane_Start", fmt.Sprintf("%d", start))
				comm.MsgBus.Send3("NeuronAPSlowGraph", "Model", "Set", "", "", "Lane_End", fmt.Sprintf("%d", end))
			} else {
				comm.MsgBus.Send2("NeuronAPSlowGraph", "Graph", "UnSelected", "Surge", fmt.Sprintf("%d", g.ID()), "")
			}
			return true, g.ID()
		}
		break
...
func (g *NeuronAPSlowGraph) SetSeries(accessor SeriesAccessor) {
}

// Destroy release resources
func (g *NeuronAPSlowGraph) Destroy() {
}

func (g *NeuronAPSlowGraph) Prep() {
}

// DrawAt renders graph to texture
func (g *NeuronAPSlowGraph) Draw() {
...
func (g *NeuronAPSlowGraph) drawXticks() {
...
func (g *NeuronAPSlowGraph) Check() bool {
	if samples.Sim.NeuronAPSlowSamples == nil {
		return false
	}

	lanes := samples.Sim.NeuronAPSlowSamples.GetLanes()
	...
	if g.graphIt.First() {
		...
		g.setScanWindow(samples.Sim.NeuronAPSlowSamples)
		return true
	}
...
func (g *NeuronAPSlowGraph) setScanWindow(lanes *samples.Samples) {
...
func (g *NeuronAPSlowGraph) dataAccessor() (x, y float64, c color.Color, more int) {
