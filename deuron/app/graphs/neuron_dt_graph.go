...
func NewNeuronDtGraph(renderer *sdl.Renderer, texture *sdl.Texture, width, height int) IGraph {
	g := new(NeuronDtGraph)
...
func (g *NeuronDtGraph) Listen(msg *comm.MessageEvent) {
	if !g.selected {
		return
	}

	samples := samples.Sim.NeuronDtSamples
...
func (g *NeuronDtGraph) Handle(vx, vy int32, eventType events.MouseEventType) (handled bool, id int) {
	...
	case events.MouseButton:
		if inside {
			...
			if g.selected {
				...
				comm.MsgBus.Send2("NeuronDtGraph", "Graph", "Selected", "Surge", fmt.Sprintf("%d", g.ID()), "")
				// Update gui Range fields the sample's range.
				samples := samples.Sim.NeuronDtSamples
				start, end := samples.GetRange()

				// Send message to panel including the panel's id.
				comm.MsgBus.Send3("NeuronDtGraph", "Model", "Set", "", "", "Lane_Start", fmt.Sprintf("%d", start))
				comm.MsgBus.Send3("NeuronDtGraph", "Model", "Set", "", "", "Lane_End", fmt.Sprintf("%d", end))
			} else {
				comm.MsgBus.Send2("NeuronDtGraph", "Graph", "UnSelected", "Surge", fmt.Sprintf("%d", g.ID()), "")
			}
			return true, g.ID()
		}
		break
...
func (g *NeuronDtGraph) SetSeries(accessor SeriesAccessor) {
}

// Destroy release resources
func (g *NeuronDtGraph) Destroy() {
}

func (g *NeuronDtGraph) Prep() {
}

// DrawAt renders graph to texture
func (g *NeuronDtGraph) Draw() {
...
func (g *NeuronDtGraph) Check() bool {
	if samples.Sim.NeuronDtSamples == nil {
		return false
	}

	lanes := samples.Sim.NeuronDtSamples.GetLanes()
	...
	if g.graphIt.First() {
		...
		g.setScanWindow(samples.Sim.NeuronDtSamples)
		return true
	}
...
func (g *NeuronDtGraph) setScanWindow(lanes *samples.Samples) {
...
func (g *NeuronDtGraph) dataAccessor() (x, y float64, c color.Color, more int) {
