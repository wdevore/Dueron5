...
// StimulusScatterGraph renders spikes as bars.
// Each horizontals line is a timeline of spikes for a single
// synapse.
type StimulusScatterGraph struct {
	BaseGraph
	gui.BaseWidget
...
// NewStimulusScatterGraph renders spikes
func NewStimulusScatterGraph(renderer *sdl.Renderer, texture *sdl.Texture, width, height int) IGraph {
	g := new(StimulusScatterGraph)
...
func (g *StimulusScatterGraph) Listen(msg *comm.MessageEvent) {
...
func (g *StimulusScatterGraph) Handle(vx, vy int32, eventType events.MouseEventType) (handled bool, id int) {
...
func (g *StimulusScatterGraph) SetSeries(accessor SeriesAccessor) {
}

// Destroy release resources
func (g *StimulusScatterGraph) Destroy() {
}

// DrawAt renders graph to texture
func (g *StimulusScatterGraph) Draw() {
	...
	if g.poiSamplesVisible {
		...
		for spiked > 0 {
			if spiked == 1 {
				...
				g.DC.SetPixel(int(tx), int(ty+2))
				g.DC.SetPixel(int(tx), int(ty+3))
				g.DC.SetPixel(int(tx), int(ty+4))
				g.DC.SetPixel(int(tx), int(ty+5))
				g.DC.Fill()

				// g.DC.MoveTo(winX, py)
				// g.DC.LineTo(winX, py-6)
				// g.DC.Stroke()
			}
	...
	if g.stimSamplesVisible {
		...
		for sSpiked > 0 {
			if sSpiked == 1 {
				// if sy == g.getStartYPosition() {
				// 	g.DC.SetColor(g.unknownColor)
				// } else {
				g.DC.SetColor(sc)
				// }
...
func (g *StimulusScatterGraph) Check() bool {
	...
	if g.poisIt.First() {
		// Set start y lane line value
		g.poisLaneY = g.getStartYPosition()
		...
		if g.stimIt.First() {
			...
			// fmt.Printf("%d\n", g.stimLane.Id)
			g.stimLaneY = g.getStartYPosition()
...
func (g *StimulusScatterGraph) setScanWindow(lanes *samples.Samples) {
...
func (g *StimulusScatterGraph) setStimScanWindow(lanes *samples.Samples) {
	// If RangeSync is enabled then we use the Model's range
	sync := deuron.SimModel.GetFloat("RangeSync")
...
func (g *StimulusScatterGraph) getStartYPosition() float64 {
	return float64(g.Rect.H) - g.borderOffsetY*2
}

func (g *StimulusScatterGraph) poissonAccessor() (x, y float64, c color.Color, spiked int) {
	if g.scanIdx >= g.scanEnd {
		g.setScanWindow(samples.Sim.PoiSamples)
		// fmt.Printf("p: %d,", g.poisLane.Id)
		if !g.poisIt.Next() {
			g.poisLaneY = g.getStartYPosition()
			return 0, 0, nil, 0
		}
...
func (g *StimulusScatterGraph) stimAccessor() (x, y float64, c color.Color, spiked int) {
	if g.stimScanIdx >= g.stimScanEnd {
		g.setStimScanWindow(samples.Sim.StimSamples)
		// fmt.Printf("s: %d,", g.stimLane.Id)
		if !g.stimIt.Next() {
			g.poisLaneY = g.getStartYPosition()
			return 0, 0, nil, 0
		}
